...
1. 打开 [Releases](../../releases)，找到最新的 `android-v*` 版本
2. 在该版本的 `Assets` 中下载 `APK` 文件
3. 把 `APK` 发送到手机（微信、QQ、网盘、数据线都可以）
4. 手机上首次安装时，按提示开启“允许此来源安装应用”
5. 安装完成后打开即可，首次启动会自动预热解析接口

### 方式二：从 Actions Artifacts 下载

1. 进入 [Actions](../../actions) 页面，打开最近一次 `android-v*` tag 构建任务
2. 在任务底部 `Artifacts` 区域下载 Android 构建产物
3. 解压后得到 `APK`，传到手机安装（同上）

### 常见安装问题

- **提示“禁止安装”**：在手机设置里给当前来源（浏览器/文件管理器）开启安装权限
- **提示“签名冲突”**：先卸载旧包，或保持同一签名来源再覆盖安装
- **安装后打开闪退**：优先下载最新 `android-v*` 版本重新安装

## 使用

1. 点击浏览器工具栏中的 VIPER 图标（脉冲波 + 三角形）
2. 等待顶部状态显示接口就绪
3. 输入电视剧/动漫名称，点搜索
4. 点击剧集按钮播放；看不了点「▼」换源
...
- 亮暗主题切换：手动一键切换（浅色 Apple 风格 + 深色模式），`CompositionLocal` 驱动全局色彩联动
...
- DLNA 投屏：搜索局域网 DLNA 设备，将视频推送到智能电视 / 盒子播放；支持完整遥控（进度、音量、播放暂停、切集）
